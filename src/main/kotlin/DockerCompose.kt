...
class DockerCompose {
    private var version: Int by oneNotNull()
...
class Service(private val name: String) {
    private var image: String by oneNotNull()
...
fun <T> oneNotNull() = object : ReadWriteProperty<Any?, T> {
    ...
    override fun setValue(thisRef: Any?, property: KProperty<*>, value: T) {
        if (this.value != null) {
            throw IllegalArgumentException("이 변수는 한번만 초기화 할 수 있습니다.")
        }
        this.value = value
    }
}